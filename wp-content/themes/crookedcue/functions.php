...
// Hide sidebar link
global $submenu;
unset($submenu['edit.php?post_type=global'][10]);
...
function disable_new_posts1() {
// Hide sidebar link
global $submenu;
unset($submenu['edit.php?post_type=header'][10]);

// Hide link on listing page
if (isset($_GET['post_type']) && $_GET['post_type'] == 'header') {
    echo '<style type="text/css">
    #favorite-actions, .add-new-h2, .tablenav { display:none; }
    </style>';
}
...
add_action('admin_menu', 'disable_new_posts1');

function register_my_menu() {
  register_nav_menu('new-menu',__( 'Etobicoke Menu' ));
}
add_action( 'init', 'register_my_menu' );

// hide "add new" on wp-admin menu
function raghu_admin_css1() {
global $post_type;
if($post_type == 'header') {
echo '<style type="text/css">#edit-slug-box,#view-post-btn,.page-title-action,
#post-preview,span.trash,.updated p a{display: none;}.view{display:none;}</style>';
...
add_action('admin_head', 'raghu_admin_css1');

//require get_template_directory() . '/inc/theme_options.php';


// add_filter( 'wc_product_table_shortcode_output', function( $html, $table ) {
//     $table[] = '<div class="11111111111111111111111"></div>';
//     return $table;
// }, 10, 2 );



require get_parent_theme_file_path( '/core/custom_functions.php' );
require get_parent_theme_file_path( '/core/ajax/ajax.php' );
require get_parent_theme_file_path( '/core/routs.php' );

////// Mississauga Order Menu - Shortcode
function my_mississaugafoodmenu_shortcode() {
    ob_start();
    get_template_part('/template-parts/mississauga-foodmenu');
    return ob_get_clean();   
} 
add_shortcode( 'mississauga-foodmenu', 'my_mississaugafoodmenu_shortcode' );
////// Etobicoke Order Menu - Shortcode
function my_etobicokefoodmenu_shortcode() {
    ob_start();
    get_template_part('/template-parts/etobicoke-foodmenu');
    return ob_get_clean();   
} 
add_shortcode( 'etobicoke-foodmenu', 'my_etobicokefoodmenu_shortcode' );