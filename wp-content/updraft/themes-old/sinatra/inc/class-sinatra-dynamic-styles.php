...
if ( ! class_exists( 'Sinatra_Dynamic_Styles' ) ) :
	...
	class Sinatra_Dynamic_Styles {
			...
			if ( 'background' === $type ) {
				...
				if ( 'color' === $background_type ) {
					if ( isset( $setting['background-color'] ) && ! empty( $setting['background-color'] ) ) {
						$css_buffer .= 'background-color: ' . $setting['background-color'] . ';';
					}
		...
		public function get_block_editor_css() {
			...
			// Check if single content is set to narrow.
			if ( 'post' === $post_type && 'narrow' === $single_content_width ) {

				$container_width = intval( sinatra_option( 'single_narrow_container_width' ) ) + 28;

				if ( 'boxed' === $site_layout ) {
					$container_width += 94;
				} elseif ( 'boxed-separated' === $site_layout ) {
					$container_width += 96;
				} else {
					$container_width += 2;
				}
				...
				if ( in_array( $sidebar_position, array( 'left-sidebar', 'right-sidebar' ), true ) ) {
					$sidebar_width   = sinatra_option( 'sidebar_width' );
					$container_width = ( $container_width - 100 ) * ( 100 - intval( $sidebar_width ) ) / 100;
			...
			// Headings (H1-H6).
			$css .= $this->get_typography_field_css( '.editor-styles-wrapper h1, .editor-styles-wrapper .editor-post-title__block .editor-post-title__input', 'h1_font' );
			$css .= $this->get_typography_field_css( '.editor-styles-wrapper h2', 'h2_font' );
			$css .= $this->get_typography_field_css( '.editor-styles-wrapper h3', 'h3_font' );
			...
			// 	$css .= '
			// 		#editor .editor-post-title__block .editor-post-title__input {
			// 			text-align: ' . sinatra_option( 'page_header_alignment' ) . ';
			// 		}
			// 	';
			// }
