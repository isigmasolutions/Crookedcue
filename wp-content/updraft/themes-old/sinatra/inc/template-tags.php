...
if ( ! function_exists( 'sinatra_logo' ) ) :
	...
	function sinatra_logo( $echo = true ) {
		...
		if ( sinatra_has_logo() ) {
			...
			$site_title_output = sinatra_get_logo_img_output();
...
if ( ! function_exists( 'sinatra_get_logo_img_output' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function sinatra_get_logo_img_output() {

		$output = '';

		// Grab default logo from Customizer.
		...
		if ( $default_logo ) {
			...
			$default_logo_attachment_src = wp_get_attachment_image_src( $default_logo, 'full' );
			...
			$default_logo_alt = get_post_meta( $default_logo, '_wp_attachment_image_alt', true );
			$logo_attr['alt'] = $default_logo_alt ? $default_logo_alt : sinatra_get_site_title();

			// Grab retina logo from Customizer.
			$retina_logo = sinatra_option( 'logo_default_retina' );

			// Build srcset attribute.
			if ( is_array( $retina_logo ) && isset( $retina_logo['background-image'] ) && $retina_logo['background-image'] ) {
				$srcset = ' srcset="' . esc_attr( $logo_attr['url'] ) . ' 1x, ' . esc_attr( $retina_logo['background-image'] ) . ' 2x"';
			}

			// Build logo output.
			$output = sprintf(
				'<a href="%1$s" rel="home"%3$s>
					<img src="%2$s" alt="%4$s" width="%5$s" height="%6$s" class="%8$s"%7$s%9$s/>
				</a>',
